...
from questionnaire_loader import load_questionnaires
...
# ── LLM model ────────────────────────────────────────────
# 統一定義在這裡，之後 Groq 又要下架/更換模型時，只需要改這一行。
# llama-3.3-70b-versatile 已被 Groq 宣布於 2026/08/16 下架，
# 改用官方建議的替代模型 openai/gpt-oss-120b。
...
# ── 問卷登記表（新版：從 questionnaires/*.json 讀取）────────────
# 目前只有「胸痛」（chest）已改成新格式；頭痛/腹痛暫時還是走舊版
# STEPS_HEADACHE / STEPS_ABDOMEN 寫死流程，之後陸續把 49 份問卷
# 補進 questionnaires/ 資料夾後，會自動被載入、自動切換成新版流程，
# 不需要再改這個檔案。
QUESTIONNAIRES: dict[str, dict] = load_questionnaires()

# ── 舊版（頭痛/腹痛，尚未改成 JSON 檔）的建議醫囑 ──────────────
# 之後這兩型也搬進 questionnaires/*.json 後，就會改用 JSON 裡的
# suggested_orders 欄位，這裡可以整段刪掉。
LEGACY_SUGGESTED_ORDERS: dict[str, list[str]] = {
    "headache": ["腦部電腦斷層（CT Head）"],
    "abdomen": ["腹部超音波"],
}


def get_suggested_orders(ctype: str) -> list[str]:
    """取得某個主訴類型問完之後，AI 預設建議醫師考慮的檢查／檢驗草稿清單。"""
    if ctype in QUESTIONNAIRES:
        return list(QUESTIONNAIRES[ctype].get("suggested_orders", []))
    return list(LEGACY_SUGGESTED_ORDERS.get(ctype, []))
...
try:
    ...
    # rag.py 這個檔案本身可能存在，但它 import 的某個套件（例如 chromadb、
    # sentence-transformers）還沒安裝，或是檔案裡少了某個名稱。這些跟
    # 「rag.py 檔案遺失」是不同狀況，訊息分開講清楚，才不會誤導成去找一個
    # 其實存在的檔案。
    import os as _os2
    _rag_py_path = _os2.path.join(_os2.path.dirname(_os2.path.abspath(__file__)), "rag.py")
    if not _os2.path.exists(_rag_py_path):
        print("[RAG] rag.py 檔案不存在，RAG 停用")
    elif isinstance(e, ModuleNotFoundError):
        print(f"[RAG] rag.py 缺少相依套件「{e.name}」，請先 pip install -r requirements.txt（RAG 停用）")
    else:
        print(f"[RAG] rag.py 載入失敗：{e}（RAG 停用）")
...
# ── 封閉式答案（選項固定）的常見口語別名／同音字快速對照表 ─────
# 這是「免費、免呼叫 LLM」的第一輪比對用的，命中就不用多花一次 API call。
# 之後任何題目只要答案是固定選項（性別、是否、左右...），都可以在這裡
# 加一組 "正確選項": [常見說法/同音字, ...]，不用再各自寫一套判斷邏輯。
CHOICE_QUICK_ALIASES: dict[str, list[str]] = {
    "男": ["男性", "男生", "先生", "男的", "南", "難", "藍"],   # 「男」常見誤聽
    "女": ["女性", "女生", "小姐", "女的", "努", "怒", "呂"],   # 「女」常見誤聽
}
...
def _quick_choice_match(raw_text: str, options: list[str]) -> str | None:
    """免費、不呼叫 LLM 的第一輪比對：選項本身或常見同音字/口語別名有沒有直接出現在原始文字裡。"""
    corrected = correct_transcription(raw_text)
    for opt in options:
        aliases = CHOICE_QUICK_ALIASES.get(opt, []) + [opt]
        if any(a in corrected or a in raw_text for a in aliases):
            return opt
    return None


def normalize_choice_answer(question_text: str, raw_text: str, options: list[str]) -> str | None:
    """
    通用的「封閉式答案」語音校正機制。適用於任何答案只會落在固定選項裡的題目
    （性別、是否、左右...等），不用再為每一題手動維護一份同音字清單。

    流程：
    1. 先用免費的關鍵字/同音字比對（_quick_choice_match）試著命中，
       涵蓋率最高、最常見的誤聽（例如「男」被聽成「南」）在這一步就解決，不花 LLM 額度。
    2. 沒命中的話，才呼叫 LLM——把「這一題在問什麼」＋「合法選項有哪些」＋「聽到的原始文字」
       一起交給模型判斷。重點是給模型「這題只可能是這幾個答案之一」的上下文，
       這樣就算是快速比對表沒收錄過的誤聽字，LLM 也能靠語境推斷回正確選項，
       不用每出現一種新的誤聽就手動加一條規則。
    3. 如果 LLM 也判斷不出來（或呼叫失敗），回傳 None，維持現有「請再說一次」的重問機制，
       不會硬塞一個猜測的答案進病歷。
    """
    quick = _quick_choice_match(raw_text, options)
    if quick:
        return quick
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "你是語音問診系統的答案校正模組。使用者的回答是語音轉文字的結果，"
                        "可能因為口音或收音品質不佳，而被誤聽成同音或音近的字。\n"
                        f"這一題問的是：「{question_text}」\n"
                        f"合法的答案只有以下幾種：{', '.join(options)}\n"
                        "請判斷使用者的原始輸入最接近以上哪一個選項，"
                        "只能回傳選項清單裡的其中一個字串，一字不差；"
                        "如果完全無法判斷屬於哪一個選項，回傳 UNKNOWN。不要有其他任何文字。"
                    ),
                },
        ...
        traceback.print_exc()
        print(f"[Normalize] LLM 校正失敗，改用重問機制: {e}")
        return None
...
COMMON_QUESTIONS = {
    14: "平時是否有抽菸習慣，還是過去曾抽但已戒菸呢？",
    16: "過去是否有下列慢性疾病史，並將有的病史告訴我：糖尿病、 慢性腎病、 高血脂、 肝硬化、 自體免疫疾病、 癌症、 其他、 如果都沒有，請說「以上皆無」。如果得過自體免疫疾病或癌症，請說出具體病名。",
    17: "您提到了需要進一步說明的疾病，請簡單描述一下病名。",
    18: "之前是否曾接受過以下藥物治療，並將有接受過的告訴我：抗組織胺、 腎上腺素、 類固醇、 如果都沒有，請說「以上皆無」。",
    20: "您提到了其他手術，請簡單說明手術名稱。",
    21: "目前是否有正在服用的藥物？若有，請告知藥物名稱。",
    22: "是否有特殊藥物和食物過敏？若有，請描述對哪些特殊藥物或食物過敏。",
}
...
CHEST_QUESTIONS = {
    ...
    8: "胸痛的位置是否會移動，還是痛點固定在同一個地方呢？",
    9: "是否有觸痛點？也就是用手按壓那個部位時，會不會有壓痛感？",
    10: "你會如何描述這種疼痛？例如刺痛、鈍痛，還是感覺有重物壓迫呢？",
    11: "你有觀察到哪些情況會使疼痛加重嗎？比方說深呼吸、耗費體力的活動，或是感到有壓力的時候？",
    12: "你有觀察到什麼情況能緩解疼痛嗎？比方說休息、用藥、坐姿，或按摩疼痛部位的時候？",
    13: "請幫我觀察是否出現以下症狀，並將有出現的症狀念給我聽：感到呼吸急促、 冒冷汗、 感到噁心或已經嘔吐、 有昏厥要暈倒或頭暈的經歷、 感到心跳加速或心跳不規則、 咳嗽有痰、 肚子痛、 如果都沒有，請說「以上皆無」。",
    15: "過去是否有下列心肺疾病，並將過去有的心肺疾病描述給我聽：高血壓、 心絞痛、 心臟衰竭、 心肌梗塞、 心律不整、 主動脈剝離、 肺栓塞、 肺高壓、 心包膜積水、 氣喘、 肺癌、 慢性阻塞型肺病、 支氣管擴張、 氣胸、 中風、 如果都沒有，請說「以上皆無」。",
    19: "是否接受過下列手術，並將過去有接受過的手術描述給我聽：心臟支架、 心臟血管繞道手術、 主動脈人工血管置換、 主動脈支架、 心律調節器、 氣胸胸腔鏡手術、 腦部手術、 水腦引流、 頸動脈手術、 腦部放射線治療、 頸椎手術、 其他手術，請描述手術名稱、 如果未曾手術，請說「未曾手術」。",
}

HEADACHE_QUESTIONS = {
    4: "這次的頭痛，大概是從什麼時候開始的？",
    5: "",
    6: "這次的頭痛，是像被雷擊般在幾秒內就痛到最劇烈，還是慢慢加重的呢？",
    7: "頭痛的位置主要在哪裡？是單側、兩側都痛、前額、後腦勺及頸部，還是整個頭呢？",
    8: "這是您這輩子最嚴重、最劇烈的一次頭痛嗎，還是跟以前差不多或較輕呢？",
    9: "您會怎麼描述這個頭痛的感覺？例如像脈搏一樣的跳痛、悶脹痛、像被緊緊束住，還是針刺般的刺痛呢？",
    10: "什麼情況下，頭痛會比較嚴重？比方說咳嗽或用力、彎腰低頭、身體活動、光線刺激，或是聲音刺激？",
    11: "什麼情況下，頭痛會比較緩解？比方說休息、使用止痛藥、待在黑暗安靜的地方，或按摩頭頸部？",
    12: "除了頭痛，您還有以下哪些症狀，請告訴我：噁心或嘔吐、 畏光、 畏聲、 視力模糊或複視、 頸部僵硬合併發燒、 單側肢體無力或麻木、 講話不清楚、 意識改變或嗜睡、 如果都沒有，請說「以上皆無」。",
    13: "請問是否有以下情形，請告訴我：近期頭部外傷、 癌症病史或免疫功能低下、 目前服用抗凝血藥物、 懷孕或產後六週內、 如果都沒有，請說「以上皆無」。",
    15: "過去是否有以下神經血管相關疾病史，請告訴我：中風、 腦動脈瘤、 腦出血、 腦膜炎或腦炎、 腦部腫瘤、 癲癇、 偏頭痛病史、 顳動脈炎、 如果都沒有，請說「以上皆無」。",
    19: "過去是否曾接受過手術，請告訴我：腦部手術、 腦動脈瘤夾閉或栓塞手術、 水腦引流、 頸動脈手術、 腦部放射線治療、 頸椎手術、 其他手術、 如果未曾手術，請說「未曾手術」。",
}

ABDOMEN_QUESTIONS = {
    4: "肚子痛是從什麼時候開始的？",
    5: "",
    6: "請問肚子痛的性質為何？例如鈍痛、刺痛、陣痛、持續痛、由前痛到背後、由肚臍周圍痛轉移到右下腹疼痛，或是飢餓時會加劇疼痛呢？",
    7: "請問是肚子痛下列哪個位置？右上腹、左上腹、右下腹、左下腹、全腹痛、左側腰痛、右側腰痛，還是肚臍以下腹痛呢？",
    8: "請問是否有下列合併症狀，請告訴我：腹瀉、 發燒發冷、 噁心、 嘔吐、 便秘、 血便、 冒冷汗、 胃酸逆流、 血尿、 月經過期、 陰道分泌物增加、 呼吸道症狀、 如果都沒有，請說「以上皆無」。",
    9: "是否有家中或是同行的人有相同的症狀呢？",
    15: "請問你有過下列病史嗎？請告訴我：肝膽結石、 腎結石、 盲腸炎、 腸阻塞、 胰臟炎、 腹主動脈瘤、 紫質症、 糖尿病酮酸中毒、 如果都沒有，請說「以上皆無」。",
    19: "請問你有接受過下列腹部手術嗎？請告訴我：剖腹產、 闌尾切除、 子宮切除、 膽囊切除、 大腸切除手術、 胃切除手術、 其他手術、 如果未曾手術，請說「未曾手術」。",
}
...
def _question_condition_met(question: dict, data: dict) -> bool:
    """判斷某一題是否符合被詢問的條件（沒有 condition 就一定要問）。"""
    cond = question.get("condition")
    if not cond:
        return True
    field_value = str(data.get(cond.get("field", ""), ""))
    keywords = cond.get("contains_any", [])
    return any(kw in field_value for kw in keywords)


def get_dynamic_question(ctype: str, idx: int) -> dict | None:
    """取得新版問卷（QUESTIONNAIRES）中，某個 index 位置的題目定義。"""
    questions = QUESTIONNAIRES.get(ctype, {}).get("questions", [])
...
def get_current_question_text(session: dict) -> str:
    """取得目前這個 session 卡在哪一題，回傳題目文字。同時支援新版(動態問卷)跟舊版(寫死步驟)。"""
    data = session["data"]
    ctype = data.get("type", "chest")
    if session.get("engine") == "dynamic":
        q = get_dynamic_question(ctype, session.get("q_index", 0))
        return q["text"] if q else ""
    return get_question(session.get("step", 0), ctype)


def _finalize_report(data: dict, ctype: str) -> tuple[str, str]:
    """
    問卷（不論新版或舊版）問完之後共用的收尾流程：
    呼叫 LLM 生成初步評估報告、存進 patient_records、產生看診編號。
    回傳 (要回給病人的文字, 看診編號)。
    """
    try:
        response = client.chat.completions.create(
    ...
    patient_records[queue_number] = {
        ...
        "data": data,
        "ts": time.time(),
        # ── 醫囑確認機制（human-in-the-loop）──────────────
        # pending_orders：AI 給的建議草稿，僅供參考，不會自動生效
        # orders_confirmed / confirmed_orders：要等醫師在醫師端按下
        # 「確認醫囑」（/doctor/orders/confirm）才會被設定
        "pending_orders": get_suggested_orders(ctype),
        "orders_confirmed": False,
        "confirmed_orders": [],
        "orders_confirmed_ts": None,
    }
    ...
    return reply, queue_number


def handle_dynamic_answer(session: dict, req: "ChatRequest", user_input: str) -> dict:
    """
    新版問卷引擎：處理「使用者剛回答了一題」這件事。
    跟舊版 next_step() 的差別是：這裡不是靠寫死的數字跳來跳去，
    而是照 QUESTIONNAIRES[ctype]["questions"] 這個清單的順序往下走，
    並且用 _question_condition_met() 通用地判斷要不要跳過某一題。
    """
    data = session["data"]
    ctype = data.get("type", "chest")
    idx = session.get("q_index", 0)

    prev_q = get_dynamic_question(ctype, idx)
    if prev_q is not None:
        if prev_q.get("type") == "onset":
            data["onset_num"], data["onset_unit"] = parse_onset(user_input)
        elif prev_q.get("options"):
            # 封閉式選項題目（例如未來的「是否」「左右」類問題）：
            # 套用跟性別辨識同一套通用校正機制，答案只可能落在 options 裡。
            normalized = normalize_choice_answer(prev_q["text"], user_input, prev_q["options"])
            if normalized is None:
                return {
                    "reply": f"抱歉，我沒有聽清楚，請再說一次。\n\n{prev_q['text']}",
            ...
            data[prev_q["key"]] = normalized
        else:
            data[prev_q["key"]] = user_input

    questions = QUESTIONNAIRES.get(ctype, {}).get("questions", [])
    nxt_idx = idx + 1
    while nxt_idx < len(questions) and not _question_condition_met(questions[nxt_idx], data):
        nxt_idx += 1

    if nxt_idx < len(questions):
        session["q_index"] = nxt_idx
        return {
            "reply": questions[nxt_idx]["text"],
            "session_id": req.session_id,
            "completed": False,
    ...
    # 這份問卷已經問完最後一題，走共用的收尾流程
    session["step"] = -1
    reply, queue_number = _finalize_report(data, ctype)
    return {
        "reply": reply,
        "session_id": req.session_id,
        "completed": True,
...
def parse_gender(text: str) -> str | None:
    return normalize_choice_answer("請問您的性別是？", text, GENDER_OPTIONS)
...
def _seed_test_patient():
    ...
    patient_records[TEST_PATIENT_QUEUE_NUMBER] = {
        ...
        "data": test_data,
        "ts": time.time(),
        "pending_orders": get_suggested_orders("chest"),
        "orders_confirmed": False,
        "confirmed_orders": [],
        "orders_confirmed_ts": None,
    }
...
class ConfirmOrdersRequest(BaseModel):
    """醫師端「確認醫囑」用的請求格式。orders 是醫師最終確認（可能已經勾掉/改過）的清單，
    不一定等於 AI 原本建議的 pending_orders——例如醫師可能覺得不需要影像檢查而拿掉一項。"""
    queue_number: str
    orders: list[str] = []
...
async def chat(req: ChatRequest):
    ...
    if req.session_id not in sessions:
        sessions[req.session_id] = {
            "step": 0, "data": {}, "ts": time.time(),
            "engine": "legacy", "q_index": 0,
        }
        return {
    ...
    if is_junk(user_input):
        ...
        current_q = get_current_question_text(session)
    ...
    if step == 0:
        ...
        if ctype in QUESTIONNAIRES:
            # 新版：問卷是從 questionnaires/*.json 讀進來的，逐題往下走
            session["engine"] = "dynamic"
            session["q_index"] = 0
            session["step"] = 4  # 沿用「>=4 代表已進入分類問卷」的慣例，純粹相容用途
            label = QUESTIONNAIRES[ctype].get("label", ctype)
            first_q = get_dynamic_question(ctype, 0)
            reply = f"了解，您說的是{label}的問題。\n\n{first_q['text'] if first_q else ''}"
        elif ctype in ("chest", "headache", "abdomen"):
            session["step"] = 4
    ...
    elif session.get("engine") == "dynamic":
        return handle_dynamic_answer(session, req, user_input)
...
def load_patient(req: LoadPatientRequest):
    ...
    return {
        ...
        "rag_enabled": RAG_ENABLED,
        "pending_orders": record.get("pending_orders", []),
        "orders_confirmed": record.get("orders_confirmed", False),
        "confirmed_orders": record.get("confirmed_orders", []),
    }
...
@app.post("/doctor/orders/confirm")
def confirm_orders(req: ConfirmOrdersRequest):
    """
    醫師端「確認醫囑」：把 AI 建議的檢查／檢驗草稿，正式標記為醫師已確認。
    這是 human-in-the-loop 的關卡——AI 只負責建議（pending_orders），
    真正要不要開、開哪幾項，最終都是醫師這一步的 orders 決定，
    AI 的建議不會自己生效。
    """
    _cleanup_patient_records()
    ...
    record["confirmed_orders"] = req.orders
    record["orders_confirmed"] = True
    record["orders_confirmed_ts"] = time.time()

    return {
        "queue_number": req.queue_number,
        "orders_confirmed": True,
        "confirmed_orders": req.orders,
    }
