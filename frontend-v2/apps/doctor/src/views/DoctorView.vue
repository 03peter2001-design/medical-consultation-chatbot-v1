...
import ConsultationBrowser from '@medical/shared/components/ConsultationBrowser.vue'
  ...
  <div class="app-shell doctor-app">
    ...
    <aside v-if="invitation || invitationError" class="invitation-banner">
      <template v-if="invitation">
        <img v-if="invitationQr" :src="invitationQr" alt="患者預問診 QR Code" />
        <div>
          <strong>患者預問診邀請已建立</strong>
          <a :href="invitation.public_url || invitation.publicUrl" target="_blank" rel="noopener">
            {{ invitation.public_url || invitation.publicUrl }}
          </a>
          <small>有效期限：{{ invitation.expires_at || invitation.expiresAt }}</small>
        </div>
        <button class="utility-button" type="button" @click="copyInvitation">複製連結</button>
        <button class="utility-button" type="button" @click="invitation = null; invitationQr = ''">關閉</button>
      </template>
      <template v-else>
        <span class="invitation-error">{{ invitationError }}</span>
...
.invitation-banner img { width: 112px; height: 112px; }
.invitation-banner > div { display: flex; min-width: 0; flex: 1; flex-direction: column; }
.invitation-banner a { overflow: hidden; color: var(--blue); text-overflow: ellipsis; white-space: nowrap; }
.invitation-banner small { color: var(--muted); }
.invitation-error { flex: 1; color: var(--danger); }
