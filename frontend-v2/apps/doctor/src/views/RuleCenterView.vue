...
import DiseaseVoteGovernance from '@medical/shared/components/DiseaseVoteGovernance.vue'
import FactLabelGovernance from '@medical/shared/components/FactLabelGovernance.vue'
import SafetyRuleGovernance from '@medical/shared/components/SafetyRuleGovernance.vue'
import { api, connectionError } from '@medical/shared/services/doctorBackend.js'
...
const authorized = ref(doctorSession.hasScope('rules:write'))
const error = ref('')
    ...
    <main class="rule-center">
      ...
      <template v-else>
        ...
        <section class="rule-section access-section">
          ...
          <div v-if="!authorized" class="readonly-note">
            目前 UCC 帳號只有規則檢視權限。如需編輯，請由管理員授予
            <code>rules:write</code> 權限。
          </div>
          <div v-else class="authorized-bar">
            <div>
              <span class="authorized-dot" />
              <strong>UCC 治理權限已驗證</strong>
              <small>可調整 Safety 規則與疾病票數；權限隨短效工作階段更新</small>
            </div>
          </div>
