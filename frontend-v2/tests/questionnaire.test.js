...
const questionnaireControl = readFileSync(
  new URL('../packages/shared/src/components/QuestionnaireControl.vue', import.meta.url),
  'utf8',
)
...
test('control exposes voice mapping and embedded layout support', () => {
  assert.match(questionnaireControl, /defineModel\('selectedOptions'/)
  assert.match(questionnaireControl, /defineExpose\(\{ applyVoiceTranscript, focus \}\)/)
  assert.match(questionnaireControl, /\.question-control-card\.embedded/)
})
